...
namespace BaGet.Protocol
{
    public class PackageMetadataService : IPackageMetadataService
    {
        ...
        public async Task<IReadOnlyList<CatalogEntry>> GetAllMetadataOrNullAsync(
            string packageId,
        ...
        {
            IEnumerable<NuGetVersion> GetVersionFromRegistrationEntry(IEnumerable<CatalogEntry> entries)
            {
        ...
        private async Task<IReadOnlyList<T>> GetAllRegistrationEntriesOrNull<T>(
            string packageId,
            Func<IEnumerable<CatalogEntry>, IEnumerable<T>> handler,
            CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            ...
            foreach (var page in packageIndex.Pages)
            {
                ...
                result.AddRange(handler(items.Select(i => i.CatalogEntry)));
            }
